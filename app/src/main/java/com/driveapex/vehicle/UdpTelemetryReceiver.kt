...
class UdpTelemetryReceiver(private val port: Int = 38901, context: Context? = null) {
    ...
    private fun liveLoop() {
        ...
        while (running) {
            ...
            // Motor RPM, in order of trust. Nothing in this app's own process can
            // read this signal on this vehicle -- the HAL listener registers
            ...
            if (frame != null || rpm != null || daemonFrame != null) {
                ...
                publish(
                    TelemetryFrame(
                        ...
                        source = when {
                            directRpm != null -> base
                            daemonRpm != null -> "$base+DIPLUS_DAEMON_RPM"
